      ...
      if (!aggregationNode->preGroupedKeys().empty() &&
        ...
        if (i < planNodes.size() - 1) {
          ...
          if (aggregationNode
            ...
            auto aggregationPtr = std::make_unique<HashAggregation>(id + 1, ctx.get(), aggregationNode);
        ...
        if (i < planNodes.size() - 2) {
          ...
          if (projectNode
            ...
            operators.push_back(std::move(expandPtr));
            operators.push_back(
              std::make_unique<FilterProject>(id + 1, ctx.get(), nullptr, projectNode));
            auto aggregationPtr = std::make_unique<HashAggregation>(id + 2, ctx.get(), aggregationNode);
...
bool DriverFactory::isRollupEnabled(
  ...
  for (auto step = 0; step < projections.size(); step++) {
    // 获取projections每个投影最后一列(gid)的值，constTypedExpr->value()获取到的类型为varint
    // 需要通过.value<int64_t>()转为数值
